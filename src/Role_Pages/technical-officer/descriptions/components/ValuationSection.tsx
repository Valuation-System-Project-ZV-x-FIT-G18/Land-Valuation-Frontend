...
const ValuationSection = ({ projectId, value, onChange }: Props) => {
  ...
  if (!v) return null
  const inputCls = 'w-40 rounded-lg border border-white/15 bg-white/5 px-3 py-1.5 text-sm text-white outline-none focus:border-accent-400/60'

  return (
    <Card className="p-5 sm:p-6">
      <h3 className="mb-1 text-sm font-bold uppercase tracking-wide text-accent-300">11. Valuation</h3>
      <p className="mb-4 text-xs text-emerald-100">Contractor's Test Method (Cost Approach)</p>
        ...
        <table className="w-full text-left text-sm text-emerald-100">
          ...
          <tbody>
            ...
            <tr className="border-t border-white/10">
              ...
              <td className="py-2 pr-3 text-right font-semibold text-accent-200">{rs(v.landValue)}</td>
              <td></td>
            </tr>
            <tr className="border-t border-white/10">
              <td className="py-2 pr-3">Market value of land and building</td>
              <td></td><td></td><td></td>
              <td className="py-2 text-right font-semibold text-accent-200">{rs(v.marketValue)}</td>
            </tr>
            <tr className="border-t border-white/10">
              <td className="py-2 pr-3 font-bold text-white">Say</td>
              <td></td><td></td><td></td>
              <td className="py-2 text-right font-bold text-accent-300">{rs(v.say)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <label className="block"><span className="mb-1 block text-[11px] text-emerald-200">Rate per perch (Rs.)</span>
          <input type="number" value={v.ratePerPerch || ''} onChange={(e) => update({ ratePerPerch: Number(e.target.value) })} className={inputCls} /></label>
        <label className="block"><span className="mb-1 block text-[11px] text-emerald-200">Building value (Rs.)</span>
          <input type="number" value={v.buildingValue || ''} onChange={(e) => update({ buildingValue: Number(e.target.value) })} className={inputCls} /></label>
        <label className="block"><span className="mb-1 block text-[11px] text-emerald-200">Say (Rs.)</span>
          <input type="number" value={v.say || ''} onChange={(e) => update({ say: Number(e.target.value) })} className={inputCls} /></label>
      </div>

      {/* Notes */}
      <div className="mt-4">
        <p className="mb-2 text-xs font-semibold text-emerald-200">Notes</p>
        <ol className="space-y-2">
          {(v.notes ?? []).map((n, i) => (
            <li key={i} className="flex gap-2">
              <span className="pt-2 text-xs text-emerald-200">{i + 1}.</span>
              <textarea value={n} onChange={(e) => setNote(i, e.target.value)} rows={i === 0 ? 1 : 3}
                className="w-full resize-none rounded-lg border border-white/15 bg-white/5 px-3 py-1.5 text-xs text-white outline-none focus:border-accent-400/60" />
            </li>
          ))}
        </ol>
      </div>
