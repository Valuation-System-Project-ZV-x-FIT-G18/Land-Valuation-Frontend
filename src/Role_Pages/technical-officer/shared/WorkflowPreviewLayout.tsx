...
const WorkflowPreviewLayout = ({ projectId, refreshToken = 0, valueOverrides = {}, valuationOverride, evidenceOverride, children }: Props) => {
  ...
  useEffect(() => {
    ...
    Promise.all([getBuildValues(projectId), getValuation(projectId), getEvidence(projectId), getPhotos(projectId)])
      .then(([buildValues, valuationData, evidenceData, photos]) => {
  ...
  const previewValues = useMemo(() => ({ ...(values ?? {}), ...valueOverrides }), [valueOverrides, values])
  ...
  return (
    <div className="grid gap-4 xl:h-[calc(100vh-8rem)] xl:min-h-[680px] xl:grid-cols-[minmax(360px,2fr)_minmax(0,3fr)]">
      ...
      <LiveReportPreview html={html} loading={loading} savedDraft={false} readiness={readiness} />
    </div>
  )
}
