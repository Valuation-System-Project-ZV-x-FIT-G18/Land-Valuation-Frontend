...
const distanceQuality = (km: number) => {
  ...
  if (km <= 2) return { label: 'Usable proximity', tone: 'border-gold-400/30 bg-gold-400/10 text-gold-200' }
...
const recencyQuality = (date: string) => {
  ...
  if (age <= 2) return { label: 'Usable ≤ 2 years', tone: 'border-gold-400/30 bg-gold-400/10 text-gold-200' }
  ...
  <Card className="overflow-hidden">
    <div className="hidden">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gold-300/75">Comparable selection</p>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-3">
        {[
          ...
          <div key={number} className="rounded-xl border border-white/10 bg-white/[0.035] p-3">
            <span className="text-[10px] font-bold text-gold-300/70">{number}</span>
            <p className="mt-1 text-xs font-semibold text-white">{title}</p>
            <p className="mt-1 text-[11px] leading-4 text-emerald-100/50">{text}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="p-5 sm:p-6">
      ...
      {comparables.length === 0 && !loading && (
        <div className="rounded-2xl border border-dashed border-white/15 bg-black/10 px-5 py-10 text-center">
          <div className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-gold-400/10 text-xl">⌖</div>
          <h5 className="mt-3 font-semibold text-white">No suitable nearby lands found yet</h5>
          <p className="mx-auto mt-1 max-w-md text-xs leading-5 text-emerald-100/55">
          ...
          return (
            <section key={i} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035]">
              <div className="flex flex-wrap items-center gap-3 border-b border-white/10 px-4 py-3">
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gold-400/15 text-xs font-bold text-gold-200">{String(i + 1).padStart(2, '0')}</span>
                <div>
                  <p className="text-sm font-semibold text-white">Nearby Land {String(i + 1).padStart(2, '0')}</p>
                  <p className="text-[11px] text-emerald-100/45">{c.area || 'Location not entered'}</p>
                </div>
              ...
              <div className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <Field label="Land type">
                  <select
                    ...
                    className="h-9 w-full rounded-lg border border-white/15 bg-emerald-950 px-3 text-sm text-white outline-none focus:border-gold-400/60"
                  >
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <span className="text-[11px] text-emerald-100/45">Use the listing link or enquiry reference when it is available.</span>
      </div>
    </div>
  </Card>
)
