...
const cardBtn = 'group flex w-full items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 p-5 text-left transition hover:border-accent-400/40 hover:bg-white/10'
...
const projectStage = (assignments: Assignment[]) => {
  ...
  if (isNew) return { label: 'New assignment', classes: 'border-accent-400/45 bg-accent-400/10 text-accent-200' }
...
const ProjectValuationPicker = ({ toId, actionLabel = 'Open', onSelect, completed = [], statusFilter, emptyText, persistenceKey, searchTerm = '', directProjectSelection = false, projectActionLabel }: Props) => {
  ...
  if (loading) return <p className="text-center text-sm text-emerald-200">Loading your projects…</p>
  if (assignments.length === 0 || projects.length === 0) return (
    <Card className="p-8 text-center">
      <p className="font-semibold text-accent-200">No projects</p>
      <p className="mt-1 text-sm text-emerald-100">{emptyText ?? (searchTerm ? 'No projects match your search.' : 'Projects assigned to you will appear here.')}</p>
    </Card>
  )
  ...
  if (project) {
    ...
    return (
      <div className="space-y-4">
        <BackButton onClick={() => setProject(null)} label="All projects" />
        <Card className="border-accent-400/25 bg-accent-400/5 p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-white">Project</p>
          <p className="mt-1 text-xl font-bold text-accent-300">{project}</p>
          <p className="mt-1 text-sm text-white">{head.owner.name} · {head.location.address || head.location.district || 'Location to be confirmed'}</p>
        </Card>
        <div className="flex items-center gap-3"><span className="h-px flex-1 bg-white/10" /><p className="text-xs font-semibold uppercase tracking-[0.16em] text-white">Select a valuation</p><span className="h-px flex-1 bg-white/10" /></div>
        {vals.map((a) => (
          <button key={a.valuationId} type="button" onClick={() => onSelect(a)} className={cardBtn}>
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-emerald-100">Valuation</p>
              <p className="mt-1 font-semibold text-white">Valuation #{a.valuationId}</p>
              <p className="mt-0.5 text-xs text-emerald-100">{a.status} · Visit {a.date || 'Not scheduled'} {a.time}</p>
            </div>
    ...
    <div className="space-y-3">
      {projects.map((list) => {
        ...
        return (
          ...
          >
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-white">Project</p>
              ...
              <p className="mt-1 truncate text-sm text-white">{a.owner.name} · {a.location.district || 'Location to be confirmed'}</p>
              {(a.date || a.time) && (
