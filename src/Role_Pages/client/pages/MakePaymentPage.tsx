        ...
        {target && (
          <div>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3 text-sm">
              ...
              <p className="mb-1 mt-3 text-[11px] font-semibold uppercase tracking-wide text-emerald-200/50">{target.reportPrice > 0 ? 'Manager-approved report price' : 'How the fee is worked out'}</p>
              <table className="w-full text-xs">
                ...
                <tbody className="text-emerald-100/85">
                  ...
                  {target.reportPrice > 0 && (
                    <tr className="border-t border-white/5"><td className="py-2 text-emerald-100/85" colSpan={2}>Price set by Manager L1 when the report was locked</td><td className="py-2 text-right font-semibold text-white">{rs(target.reportPrice)}</td></tr>
                  )}
                </tbody>
              </table>
              {target.reportPrice <= 0 && <div className="mt-2 flex justify-between border-t border-white/10 pt-2 text-emerald-100/80"><span>Total of bands (scale fee)</span><span>{rs(target.scaleFee)}</span></div>}
              {target.reportPrice <= 0 && target.minApplied && <div className="flex justify-between text-amber-300/90"><span>Minimum fee applied</span><span>{rs(target.minFee)}</span></div>}
              {target.reportPrice <= 0 && !target.minApplied && target.fee !== target.scaleFee && <div className="flex justify-between text-emerald-200/60"><span>Rounded up to nearest Rs. 100</span><span>+ {rs(target.fee - target.scaleFee)}</span></div>}
