...
const RegisterBankPage = () => {
  ...
  // Arriving from Register Applicant pre-fills the reference with the NIC.
  ...
  const [createdRef, setCreatedRef] = useState('') // ref used, for the Create Project step

  // Search existing banks by NIC or Project ID.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.bankName) return setError('Please choose a bank.')
    if (
      !form.projectRef ||
      !form.branchCode ||
      !form.officerName ||
    ...
    if (form.email && validateEmail(form.email)) return setError('Enter a valid email address.')

    setSubmitting(true)
    const res = await registerBank(form)
    setSubmitting(false)
    if (res.ok) {
      setCreatedRef(form.projectRef) // remember it for the Create Project step
      setDone(true)
      setForm(empty)
    } else setError(res.error ?? 'Could not register the bank.')
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <PageHeader
        title="Register"
        accent="Bank"
        subtitle="Add a bank branch and its officer who request valuations."
      />
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="flex-1">
            <Input
              ...
              placeholder="Applicant NIC or Project ID"
            />
          </div>
              ...
              {foundBanks.map((b) => (
                <div key={b.branchCode} className="rounded-xl border border-white/10 bg-white/5 p-3 text-sm">
                  ...
                  <p className="text-xs text-emerald-100/70">
                    Branch code {b.branchCode} · Officer {b.officerName} · Ref {b.projectRef}
                  </p>
                </div>
              ))}
            </div>
      ...
      <Modal open={done} onClose={() => setDone(false)}>
        <div className="text-center">
          ...
          <p className="mx-auto mt-2 max-w-xs text-sm text-emerald-100/70">
            The bank branch has been added successfully.
          </p>
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row">
            <Button
              ...
              fullWidth
              onClick={() =>
                navigate('/coordinator/new-project', { state: { nic: createdRef } })
              }
            >
              Yes, create project
            </Button>
            <Button type="button" variant="outline" fullWidth onClick={() => setDone(false)}>
              Register another
            </Button>
          </div>
        </div>
