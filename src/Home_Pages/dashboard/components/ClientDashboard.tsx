...
const ClientDashboard = ({ user, audience }: { user: AuthUser; audience: 'bank' | 'applicant' }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [items, activity] = await Promise.all([getClientDashboardProjects(), getNotifications()])
  ...
  return <div className="space-y-6">
    ...
    <SummaryCards loading={loading} items={isBank ? [
      { label: 'Valuation Requests', value: projects.length, hint: 'Projects associated with your branch', tone: 'bg-accent-300' },
      ...
      { label: 'My Projects', value: projects.length, hint: 'Your valuation requests', tone: 'bg-accent-300' },
