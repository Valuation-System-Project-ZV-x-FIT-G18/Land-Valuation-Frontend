  ...
  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-busy={loading}>
    {items.map((item) => (
      <Card key={item.label} className="p-5">
        ...
        <p className="mt-1 text-sm font-semibold text-emerald-100/80">{item.label}</p>
        <p className="mt-1 text-xs text-emerald-100/45">{item.hint}</p>
      </Card>
    ))}
  </div>
)
...
export const SectionCard = ({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) => (
  <Card className="overflow-hidden">
    ...
    <div className="p-5 sm:p-7">
      ...
      {subtitle && <p className="mt-1 text-sm text-emerald-100/55">{subtitle}</p>}
...
export const EmptyState = ({ children }: { children: ReactNode }) => (
  <p className="rounded-xl border border-dashed border-white/15 bg-white/[0.025] px-5 py-9 text-center text-sm text-emerald-100/60">{children}</p>
)
