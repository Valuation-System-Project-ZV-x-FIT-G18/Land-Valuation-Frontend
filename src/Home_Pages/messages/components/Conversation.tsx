  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-3 border-b border-slate-700/60 px-4 py-3">
        ...
        <span className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-700/60 text-sm font-semibold text-white">
      ...
      {/* Messages */}
      <div className="flex-1 space-y-3 overflow-y-auto bg-[#091f22] p-4 sm:p-6">
        {messages.length === 0 ? (
          ...
          messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[78%] rounded-lg border px-3.5 py-2.5 text-sm ${mine ? 'border-gold-400/20 bg-gold-400 text-emerald-950' : 'border-slate-700/70 bg-slate-800/65 text-slate-100'}`}>
      ...
      {/* Reply box */}
      <form onSubmit={submit} className="border-t border-slate-700/60 bg-[#0d292d] p-3">
        {file && (
        ...
        <div className="flex items-end gap-2">
          ...
          <textarea
            ...
            className="max-h-32 flex-1 resize-none rounded-md border border-slate-600/70 bg-slate-900/40 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 outline-none focus:border-gold-400/60 focus:ring-2 focus:ring-gold-400/20"
          />
